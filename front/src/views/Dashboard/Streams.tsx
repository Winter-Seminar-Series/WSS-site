...
const Streams = ({
  ...
  const mapEventTypeToComponent = {
    seminar: (event, index) => {
      ...
      if (!seminar) return;
      return (
        <div key={seminar.id} className="col-xs-10 col-sm-6 col-lg-3 mt-2 mb-4">
          <SeminarCard
            id={seminar.speaker}
    ...
    roundtable: (event, index) => {
      const roundTable = roundtables.find(
        (roundTable) => roundTable.id === event.id
      );
              ...
              Array.from(Array(streams.length).keys())
                ...
                .map((index) => {
                  ...
                  const eventComponent =
                    event && mapEventTypeToComponent[event.type];
                  return (
                    eventComponent && eventComponent(streams[index], index)
                  );
                })
            }
          </div>
...
const mapStateToProps = (state, ownProps) => {
  const { isFetching, streams, seminars, workshops, roundtables, labtalks } =
    state.WSS;
